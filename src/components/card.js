...
function addOrRemoveLikes(evt) {
  if (evt.target.classList.contains("element__group")) {
    evt.target.classList.contains("element__group_active")
      ? findAndDeleteHeart(evt)
      : findAndPostHeart(evt);
  }
}
...
export class Card {
  ...
  updateLikesView(someData) {
    this._getElement().querySelector(".my-like").textContent =
      someData.likes.length;
  }
}
